...
export const calculateGuidePositions = (dimensions: {left: number, top: number, width: number, height: number}, axis: Axis) => {
	if (axis === 'x') {
		const start = dimensions.left;
		const middle = dimensions.left + (dimensions.width / 2); 
		const end = dimensions.left + dimensions.width;

		return [ start, middle, end ];
	} else {
		const start = dimensions.top;
		const middle = dimensions.top + (dimensions.height / 2)
...
export const getAllGuidesForGivenAxisExceptActiveBox = (allGuides: {[key: string]: {[key: string]: any}}, guidesForActiveBoxAlongGivenAxis: any, axis : Axis) => {
...
export const checkValueProximities = (activeBoxGuidesInOneAxis: any[], allOtherGuidesInOneAxis: any[]) => {
	let proximity = null;
...
export const calculateBoundariesForDrag = ( left: number, top: number, width: number, height: number, bounds: {width: number, height: number} ) => {
...
export const calculateBoundariesForResize = (left: number, top: number, width: number, height: number, bounds: {width: number, height: number}) => {
	const boundingBox = { ...bounds };
...
export const getOffsetCoordinates = (node: any) => {
	return {
		x: node.offsetLeft,
		y: node.offsetTop,
		top: node.offsetTop,
...
export const topLeftToCenter = ({ left, top, width, height, rotateAngle }: any) => ({
	cx: left + width / 2,
	cy: top + height / 2,
	width,
	height,
...
export const centerToTopLeft = ({ cx, cy, width, height, rotateAngle }: any) => ({
	top: cy - height / 2,
	left: cx - width / 2,
	width,
	height,
...
export const getNewStyle = (type: string, rect: any, deltaW: number, deltaH: number, minWidth: number, minHeight: number) => {
...
export const getNewCoordinates = (rect: any) => {
...
export const getMultipleSelectionCoordinates = (allBoxes: any, activeBoxes: any) => {
	let selectedBoxes = [];
...
export const getGroupCoordinates = (allBoxes: any, groupedBoxes: any) => {
	let selectedBoxes = [];
	for (let box in allBoxes) {
...
const getResizeSVGCursor = (angle: any) => {
...
export const getResizeCursorCSS = (handle: any, degree = 0) => {
	let angle = degree;
...
export const checkGroupChildElementsLocked = (captions: any[]) => {
	let isLocked = true;
